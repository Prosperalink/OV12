...
export default function ContactForm() {
  ...
  const validateEmail = (email: string) => /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i.test(email.trim());
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateName(formData.name)) newErrors.name = 'Please enter a valid name (2-50 letters)';
    if (!validateEmail(formData.email)) newErrors.email = 'Please enter a valid email address';
    if (!formData.message || formData.message.trim().length < 10) newErrors.message = 'Message must be at least 10 characters';
